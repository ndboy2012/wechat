...
public class DataGridTag extends TagSupport {
	...
	public void setColumn(String title, String field, Integer width, String rowspan, 
			...
			String imgBasePath, boolean query, String url, String funname, 
			String arg,String queryMode, String dictionary,
			boolean frozenColumn,String extend,
			String style,String downloadName,boolean isAuto,String extendParams,String formatRow) {
		...
		dataGridColumn.setImgBasePath(imgBasePath);
		...
		dataGridColumn.setFormatRow(formatRow);
	...
	protected void getField(StringBuffer sb,int frozen) {
		...
		for (DataGridColumn column : columnList) {
			...
			if (column.isImage()) {
				...
				sb.append(" return '<img border=\"0\" src=\""+column.getImgBasePath()+"' + value+'\" />';}");
			}
            // 自定义显示图片
            if (column.getImageSize() != null) {
                    ...
                    sb.append(" return '<img width=\"" + tld[0]
                                    + "\" height=\"" + tld[1]
                                    + "\" border=\"0\" src=\""+column.getImgBasePath()+"' + value+'\" />';}");
                    tld = null;
            }
			...
			if(column.getFormatRow()!=null)
			{
				sb.append(",formatter:");
				sb.append(column.getFormatRow());
			}
			sb.append("}");
			// 去除末尾,
			if (i < columnList.size()) {
