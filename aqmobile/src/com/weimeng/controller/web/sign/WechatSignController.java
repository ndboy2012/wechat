package com.weimeng.controller.web.sign;
...
@Controller("wechatSignController")
@RequestMapping("web/signInggk/wechatSignController")
public class WechatSignController {

	private Logger logger = Logger.getLogger(WechatSignController.class);
    private boolean exit = false; // 判定用户是否有签到记录，默认是没有的
	
	@Resource(name = "wechatSignImpl")
	private WechatSignDao wechatSignImpl;
   
	@Resource(name = "wechatRegisterImpl")
	private WechatRegisterDao weiChatRegisterImpl;
	...
	public AjaxJson flashLoginVerify(
		...
		String msg = null;
		try {
			if (weiChatRegisterImpl.isExitOpenid(open_id)) {
				msg = "true";
				logger.info("已经登记好了");
			} else {
				msg = "false";
	...
	public AjaxJson registerInfo(
		...
		try {
			exitregisterinfo = weiChatRegisterImpl.isExitOpenid(open_id);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		...
		if (exitregisterinfo) { // 如果用户登记过了
			...
			try {
				if (wechatSignImpl.isExitOpenId(open_id)) {
					...
					signinfo.setLastSignDate(DateProcess.convertString2date("1970-01-01"));
			...
			try {
				...
				if (DateProcess.compareDate(new Date(),signinfo.getLastSignDate())) {
					// 没有签到机会了
	...
	@ResponseBody
	public AjaxJson newRandomPrize(
			@RequestParam(required = true, defaultValue = "abc") String open_id,HttpServletRequest request) {
		AjaxJson ajax = new AjaxJson();
	...
	@ResponseBody
	public AjaxJson updatePrize(
			@RequestParam(required = true, defaultValue = "") String openid,HttpServletRequest request) {
		AjaxJson ajax = new AjaxJson();
	...
	public AjaxJson listPrizeInfo() {
		...
		try {
		   Map<String,Object> map = ParseXml.parse("C://aqMobile/winner.xml", null);
	...
	public String updateWinnerlist() {
		try {
		    ...
		    ParseXml.ConvertToXmlFile("C://aqMobile/winner.xml", map);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
